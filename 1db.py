...
def extractValues(obj: Any) -> list[str]:
    if obj is None:
        return []
    if isinstance(obj, (str, int, float, bool)):
        return [str(obj)]
    if isinstance(obj, list):
        return [str(item) for sub in obj for item in extractValues(sub)]
    if isinstance(obj, dict):
        results = []
        if "member" in obj:
            results.extend(extractValues(obj["member"]))
        if "entry" in obj:
            results.extend(extractValues(obj["entry"]))
        if "#text" in obj:
        ...
        for k, v in obj.items():
            if not str(k).startswith("@"):
                results.extend(extractValues(v))
        return results
    return []

def findKeyRecursively(obj: Any, keys: list[str]) -> Any:
    ...
    for key, val in obj.items():
        if isinstance(val, dict):
            found = findKeyRecursively(val, keys)
