...
def _pan_object_search(kind: str, query: str, limit: int = 500) -> dict[str, Any]:
    ...
    try:
        ...
        if kind == "addresses":
            ...
            for r in candidates:
                if any(q in value.lower() for value in _fqdn_values(_safe_json(r["data"]))):
...
def _pan_object_search_html(data: dict[str, Any]) -> str:
    ...
    if kind in ("addresses", "services"):
        ...
        parts.append('<div class="object-result-columns">')
        for title, items in (("Addresses" if kind == "addresses" else "Services", object_items), ("Address Groups" if kind == "addresses" else "Service Groups", group_items)):
            parts.append(f'<section class="object-result-column"><div class="object-column-title"><h3>{escape(title)}</h3><span class="count">{len(items)}</span></div>')
            if not items:
                parts.append('<div class="empty column-empty">No matches.</div>')
            for x in items:
                raw = json.dumps(x.get("data", {}), indent=2, ensure_ascii=False, sort_keys=True)
                parts.append('<article class="object-result compact-result">')
                parts.append(f'<div class="object-result-head"><div><span class="item-name">{escape(x["name"])}</span> <span class="badge palo">{escape(x["category"])}</span></div><div class="meta">{escape(x["device"])}</div></div>')
                if x.get("description"):
                    parts.append(f'<div class="object-description"><b>Description:</b> {escape(x["description"])}</div>')
                if kind == "addresses" and x.get("networks"):
                    parts.append('<div class="object-detail-line"><b>Address:</b> ' + " ".join(f'<span class="rule-tag highlight">{escape(v)}</span>' for v in x["networks"]) + '</div>')
                if kind == "services":
                    if x.get("protocol"):
                        parts.append(f'<div class="object-detail-line"><b>Protocol:</b> <span class="rule-tag highlight">{escape(x["protocol"])}</span></div>')
                    if x.get("ports"):
                        parts.append('<div class="object-detail-line"><b>Port:</b> ' + " ".join(f'<span class="rule-tag highlight">{escape(v)}</span>' for v in x["ports"]) + '</div>')
                if x.get("members"):
                    parts.append('<div class="object-members"><b>Members:</b><div>' + " ".join(f'<span class="rule-tag">{escape(v)}</span>' for v in x["members"]) + '</div></div>')
                if x.get("match_type"):
                    parts.append(f'<div class="meta object-match">Matched by: {escape(x["match_type"])}</div>')
                parts.append(f'<details><summary>Expand full JSON</summary><pre>{escape(raw)}</pre></details></article>')
    ...
    for x in results:
        raw = json.dumps(x.get("data", {}), indent=2, ensure_ascii=False, sort_keys=True)
        parts.append('<article class="object-result">')
        parts.append(f'<div class="object-result-head"><div><span class="item-name">{escape(x["name"])}</span> <span class="badge palo">{escape(x["category"])}</span> <span class="badge">{escape(x["match_type"])}</span></div><div class="meta">Device: <b>{escape(x["device"])}</b></div></div>')
        if x.get("description"): parts.append(f'<div class="object-description"><b>Description:</b> {escape(x["description"])}</div>')
        if kind == "applications":
            if x.get("protocol"): parts.append(f'<div class="object-detail-line"><b>Protocol:</b> {escape(x["protocol"])}</div>')
            if x.get("ports"): parts.append('<div class="object-detail-line"><b>Ports:</b> ' + " ".join(f'<span class="rule-tag highlight">{escape(v)}</span>' for v in x["ports"]) + '</div>')
        if kind == "custom_urls" and x.get("fqdn_values"):
            parts.append('<div class="object-detail-line"><b>URL / FQDN:</b> ' + " ".join(f'<span class="rule-tag highlight">{escape(v)}</span>' for v in x["fqdn_values"]) + '</div>')
        parts.append(f'<details><summary>Expand full JSON</summary><pre>{escape(raw)}</pre></details></article>')
    return "".join(parts)
