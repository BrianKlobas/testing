    ...
    try:
        ...
        for page in paginator.paginate(ParentId=parent_id):
            for acc in page.get("Accounts", []):
                # Collect every tag attached to the account. Tag names are not
                # supplied as command-line arguments; retrieve everything AWS
                # Organizations exposes for the account.
                tags = {}
                try:
                    tag_paginator = client.get_paginator("list_tags_for_resource")
                    for tag_page in tag_paginator.paginate(ResourceId=acc["Id"]):
                        for tag in tag_page.get("Tags", []):
                            key = tag.get("Key")
                            if key:
                                tags[key] = tag.get("Value", "")
                except ClientError as tag_error:
                    # Keep the account even if tag retrieval fails.
                    print(f"[!] Error collecting tags for account {acc['Id']}: {tag_error}")
...
def main():
    parser = argparse.ArgumentParser(description="Export AWS Org Structure and Account Tags to JSON")
