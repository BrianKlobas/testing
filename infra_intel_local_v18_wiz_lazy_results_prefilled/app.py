...
def _security_group_details(data: Any) -> str:
    ...
    meta_rows = []
    for label, key in (("Group ID", "GroupId"), ("Group Name", "GroupName"), ("VPC ID", "VpcId"), ("Owner / Account ID", "OwnerId")):
        if raw.get(key) not in (None, ""):
            meta_rows.append(f'<tr><th>{label}</th><td><b>{_e(raw.get(key))}</b></td></tr>')
    tags = raw.get("Tags")
    if isinstance(tags, list) and tags:
        ...
        for tag in tags:
            if isinstance(tag, dict):
                ...
                v = tag.get("Value") if tag.get("Value") is not None else tag.get("value")
                if k is not None:
                    tag_values.append(f'{_e(k)}={_e(v if v is not None else "")}')
        if tag_values:
            meta_rows.append(f'<tr><th>Tags</th><td>{" ".join(f"<span class=\"rule-tag highlight\">{x}</span>" for x in tag_values)}</td></tr>')
    elif isinstance(tags, dict) and tags:
        tag_values = [f'{_e(k)}={_e(v)}' for k,v in tags.items()]
        meta_rows.append(f'<tr><th>Tags</th><td>{" ".join(f"<span class=\"rule-tag highlight\">{x}</span>" for x in tag_values)}</td></tr>')
    metadata = '<table class="prop-table" style="margin-bottom:8px;">' + ''.join(meta_rows) + '</table>' if meta_rows else f'<div class="meta" style="margin-bottom:8px;"><b>Description:</b> {_e(desc)}</div>'
    if meta_rows and desc not in (None, ""):
        metadata += f'<div class="meta" style="margin-bottom:8px;"><b>Description:</b> {_e(desc)}</div>'
...
def _org_tree(node, role_name="", account_query="", stats=None):
    """Render the organization tree, optionally filtering by account name.

    The collector normally stores accounts in ``Accounts`` and OUs in ``OUs``.
    This renderer is intentionally tolerant of the alternate ``Children`` /
    ``AccountList`` forms used by older topology files so the search works
    against existing local org_topology.json files as well.
    """
    if not isinstance(node, dict):
        return ""

    query = str(account_query or "").strip().lower()

    def account_name(acc):
        return (acc.get("Name") or acc.get("name") or acc.get("AccountName") or
                acc.get("accountName") or acc.get("DisplayName") or acc.get("displayName") or
                acc.get("Id") or acc.get("AccountId") or "Account")

    def account_id(acc):
        return acc.get("Id") or acc.get("AccountId") or acc.get("account_id") or ""

    def is_account_record(value):
        if not isinstance(value, dict):
            return False
        kind = str(value.get("Type") or value.get("type") or value.get("NodeType") or "").lower()
        if kind in {"account", "aws account"}:
            return True
        # AWS account records have an Id/AccountId and a Name but do not have
        # OU collections. Avoid treating ordinary OU/root nodes as accounts.
        return bool(account_id(value) and account_name(value) and not (
            value.get("Accounts") or value.get("AccountList") or value.get("OUs") or
            value.get("SubOUs")
        ))

    name = node.get("Name") or node.get("name") or node.get("Id") or node.get("id") or "Unit"
    accounts = node.get("Accounts") or node.get("accounts") or node.get("AccountList") or []
    children = node.get("OUs") or node.get("ous") or node.get("Children") or node.get("children") or node.get("SubOUs") or []
    ...
    # Some older exports put account objects directly under Children. Pull
    # those out as accounts and leave actual OU/resource nodes as children.
    child_accounts = []
    real_children = []
    for child in children:
        if is_account_record(child):
            child_accounts.append(child)
        else:
            real_children.append(child)
    accounts = [a for a in accounts if isinstance(a, dict)] + child_accounts
    ...
    for acc in accounts:
        ...
        # Primary search is account name, but accepting the account ID makes
        # the page useful when an operator has the 12-digit ID in hand.
        searchable = f"{acc_name} {acc_id}".lower()
        if not query or query in searchable:
            matching_accounts.append(acc)

    child_html = []
    for child in children:
        rendered = _org_tree(child, role_name, account_query, stats)
        if rendered:
            child_html.append(rendered)

    if query and not matching_accounts and not child_html:
        return ""

    if stats is not None and query:
        stats["accounts"] = stats.get("accounts", 0) + len(matching_accounts)
        if matching_accounts or child_html:
            stats["ous"] = stats.get("ous", 0) + 1
    ...
    if matching_accounts:
        ...
        for acc in matching_accounts:
            ...
            if not isinstance(tags, dict):
                tags = {}
            primary = (acc.get("PrimaryOwner") or acc.get("primaryOwner") or
                       tags.get("PrimaryOwner") or tags.get("primaryOwner") or
                       tags.get("Primary Owner") or "")
            secondary = (acc.get("SecondaryOwner") or acc.get("secondaryOwner") or
                         tags.get("SecondaryOwner") or tags.get("secondaryOwner") or
                         tags.get("Secondary Owner") or "")
    ...
    if child_html:
        parts.append('<div style="margin-left:10px;">' + "".join(child_html) + '</div>')
    return "".join(parts) + "</div>"
...
def _load_wiz_issues() -> tuple[list[dict[str, Any]], list[str]]:
    """Load Wiz issues from the normalized SQLite inventory only.

    wiz_data/*.json is ingestion input, not a runtime data source. The Wiz page
    and its search/filter choices should always read the database populated by
    ingest.py so the web application does not repeatedly parse the raw files.
    """
    try:
        conn = get_db(DB_PATH)
        try:
            table = conn.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='wiz_issues'"
            ).fetchone()
            if not table:
                return [], []
            rows = conn.execute(
                "SELECT raw_json, source_file FROM wiz_issues ORDER BY issue_id"
            ).fetchall()
...
def _aws_security_group_index() -> dict[str, list[dict[str, Any]]]:
    """Build an exact SG-ID -> AWS record index from the existing AWS inventory.

    Wiz owns the finding relationship; AWS inventory remains authoritative for
    the actual SG configuration (rules, descriptions, tags, VPC, etc.).
    """
    ...
    try:
        ...
        try:
            rows = conn.execute("""
                ...
                FROM records r JOIN devices d ON d.id=r.device_id
                WHERE r.platform='aws'
                  AND lower(r.name) LIKE 'sg-%'
            """).fetchall()
            for row in rows:
                item = _safe_json(row["data"])
                if not _is_sg(row["category"], item):
                    continue
                gid = str(item.get("GroupId") or row["name"] or "").strip()
                if not gid.lower().startswith("sg-"):
                    continue
                rec = _record(row, reason="wiz_security_group_id_correlation", matched_value=gid)
                out.setdefault(gid.lower(), []).append(rec)
        finally:
            conn.close()
    except sqlite3.Error:
        return {}
    # Avoid duplicate copies of the same SG record while preserving distinct
    # AWS source/device records when they genuinely differ.
    for key, records in list(out.items()):
        out[key] = _dedupe(records)
    return out


def _wiz_attach_aws_security_groups(results: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Attach authoritative AWS SG records to normalized Wiz findings."""
    index = _aws_security_group_index()
    for result in results:
        gid = str(result.get("security_group_id") or "").strip().lower()
        aws_records = index.get(gid, [])
        result["aws_security_groups"] = aws_records
        # Useful summary fields come from AWS when available.
        if aws_records:
            raw = aws_records[0].get("data") if isinstance(aws_records[0].get("data"), dict) else {}
            result["aws_group_name"] = raw.get("GroupName") or aws_records[0].get("name") or ""
            result["aws_vpc_id"] = raw.get("VpcId") or ""
            result["aws_description"] = raw.get("Description") or ""
            result["aws_owner_id"] = raw.get("OwnerId") or ""
        else:
            result["aws_group_name"] = ""
            result["aws_vpc_id"] = ""
            result["aws_description"] = ""
            result["aws_owner_id"] = ""
    return results
...
def _wiz_normalize_issues(issues: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return _wiz_attach_aws_security_groups(out)
...
def aws_org_page():
    ...
    account_query=request.args.get("q","").strip()
    tree=""
    stats={"accounts":0,"ous":0}
    if data is not None:
        root_nodes=data.get("Hierarchy",data) if isinstance(data,dict) else data
        if isinstance(root_nodes,list):
            tree="".join(_org_tree(x,role,account_query,stats) for x in root_nodes)
        else:
            tree=_org_tree(root_nodes,role,account_query,stats)
    if data is None:
        org_meta="Unavailable"
    elif account_query:
        n=stats.get("accounts",0)
        org_meta=f'{n} matching account{"s" if n!=1 else ""}'
    else:
        org_meta="Loaded"
    return render_template("aws_org_topology.html",active_page="org",org_tree=Markup(tree),org_error=error,role=role,
                           account_query=account_query,org_meta=org_meta,org_filter_count=stats.get("accounts",0))
...
def wiz_policy_check_page():
    ...
    has_query = any((query_ou, query_account, query_sg, query_policy))

    # Do not load/normalize the Wiz dataset just by navigating to the page.
    # Wiz data is loaded only after the user submits at least one search value.
    issues: list[dict[str, Any]] = []
    files: list[str] = []
    ...
    # The filter choices are still populated from the normalized DB-backed Wiz
    # inventory, but the actual result cards are only rendered after a query is
    # submitted. This preserves the convenient prefilled filters without making
    # the first navigation to the page show the entire findings dataset.
    all_for_filters = all_results
    ou_options = sorted({str(x.get("ou") or "") for x in all_for_filters if x.get("ou")}, key=str.lower)
    account_options = sorted(
        {(str(x.get("account_id") or ""), str(x.get("account_name") or ""))
         for x in all_for_filters if x.get("account_id")},
        key=lambda x: (x[1].lower(), x[0])
    )
    policy_options = sorted(
        {(str(x.get("policy_id") or ""), str(x.get("policy_name") or ""))
         for x in all_for_filters if x.get("policy_id") or x.get("policy_name")},
        key=lambda x: (x[1].lower(), x[0])
    )
    return render_template("wiz_policy_check.html", active_page="wiz", query_ou=query_ou, query_account=query_account,
                           ...
                           has_query=has_query, ou_options=ou_options, account_options=account_options, policy_options=policy_options)
